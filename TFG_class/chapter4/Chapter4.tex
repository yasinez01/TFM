...
El proyecto se estructura siguiendo una arquitectura por capas\footnote{\href{https://reactiveprogramming.io/blog/es/estilos-arquitectonicos/capas}{https://reactiveprogramming.io/blog/es/estilos-arquitectonicos/capas}}, lo que permite organizar el código y separar las responsabilidades de forma clara y eficiente. Cada capa cumple un papel distinto. La presentación se encarga de la interfaz y la interacción con el usuario, la aplicación contiene la lógica de negocio, la infraestructura gestiona el acceso a recursos externos como la base de datos y la capa de dominio define las reglas y entidades principales del modelo.
La elección de esta arquitectura se debe a que proporciona una base sólida y escalable para el desarrollo, favorece la mantenibilidad del sistema y facilita la incorporación de nuevas funcionalidades sin afectar al resto de componentes. Además, permite una mayor reutilización del código y mejora la calidad del software al fomentar un diseño más limpio, modular y fácilmente testeable.
\begin{figure} [h!btp]
	\centering
	\includegraphics[width=0.7\textwidth]{fig/arquitecturaCapas.png}
	\caption[Diagrama de una arquitectura por capas.]{Diagrama de una arquitectura por capas.}
	\label{fig:ej0}
\end{figure}
...
public async Task<IActionResult> GetListadoFormaciones([DataSourceRequest] DataSourceRequest request, FormacionFiltrosDTO filtros)
{
	...
	return Json(formacionesRespuesta.Data.ToDataSourceResult
	(request, ModelState));
}	
...
public async Task<IActionResult> Detalle(int formacionID)
{
	...
	{
		TempData["NotificacionMensaje"] = NotificacionMensaje.FormacionObtenerError
		.ToString();
	...
	viewModel.FormacionGeneral = viewModel.FormacionGeneral.GetViewModel
	(formacion.Data);
