...
En la pestaña Calendario, que vemos en la Figura \ref{fig:ej13}, se incluirá los datos relativos a días y horas que abarca la formación.
\begin{figure}[H]
	\centering
...
\begin{figure}[H]
	...
	\resizebox{0.9\textwidth}{!}{%
		\begin{tikzpicture}[
			...
			every node/.style={rectangle, draw, rounded corners, align=center, font=\footnotesize, minimum width=3.5cm, minimum height=0.9cm},
			arrow/.style={-Stealth, thick}
			]
			
			\node (trabajador) {Trabajador crea la acción formativa};
			\node (responsable) [below=of trabajador] {Responsable de área analiza \\ (Aprueba / Rechaza)};
			\node (responsableAprueba) [below=of responsable] {Aprobada → Enviar al director de departamento};
			\node (director) [below=of responsableAprueba] {Director de departamento analiza \\ (Aprueba / Rechaza)};
			\node (directorAprueba) [below=of director] {Aprobada → Enviar a PyT};
			\node (pyt) [below=of directorAprueba] {PyT revisa y tramita la formación};
			
			\draw[arrow] (trabajador) -- (responsable);
			
			\node (rechazado1) [left=1.8cm of responsable] {Rechazada \\ → Notifica al creador de la solicitud};
			\draw[arrow] (responsable) -- (rechazado1);
			\draw[arrow] (responsable) -- (responsableAprueba);
			\draw[arrow] (responsableAprueba) -- (director);
			
			\node (rechazado2) [left=1.8cm of director] {Rechazada \\ → Notifica al responsable \\ y creador de la solicitud};
			\draw[arrow] (director) -- (rechazado2);
			\draw[arrow] (director) -- (directorAprueba);
